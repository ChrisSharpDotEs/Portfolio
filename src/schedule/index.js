...
class Temporizador {
    ...
    init() {
        ...
        if (this.temporizerStorage.existStorage()) {
            ...
            if (ultimoRegistro) {
                const fechaInicio = ultimoRegistro.dia.split('/').map(h => h.padStart(2, '0')).reverse().join('-');
...
function setHighResolutionCanvas(canvas, width, height, scale = 1) {
...
function drawCustomCircle(canvasId, options) {
    const canvas = document.getElementById(canvasId);

    // Ajustar resolución del canvas
    const ctx = setHighResolutionCanvas(canvas, canvas.width, canvas.height);
    ...
    const {
        ...
        percentage = 80,
        primaryColor = 'orange',
        primaryThicknessBase = 20,
        secondaryColor = 'gray',
        secondaryThicknessBase = 10,
        text = `${percentage}%`,
        textColor = 'white',
        textSize = 24,
        rotation = 0,
    ...
    const primaryThickness = primaryThicknessBase * scale;
    const secondaryThickness = secondaryThicknessBase * scale;
    ...
    // Dibujar círculo secundario
    ctx.beginPath();
    ctx.arc(x, y, radius, filledAngle, 2 * Math.PI);
    ctx.lineWidth = secondaryThickness;
    ctx.strokeStyle = secondaryColor;
    ctx.lineCap = 'butt';
    ctx.stroke();

    // Dibujar círculo primario
    ctx.beginPath();
    ctx.arc(x, y, radius, 0, filledAngle);
    ...
    // Dibujar texto
    ctx.font = `${textSize * scale}px Arial`;
...
function getPercentage(time) {
    if (time == null || time.hora_fin || !time.hora_inicio) return 0;

    let fecha = time.dia.split('/').reverse().map(item => item.padStart(2, '0')).join('-');

    const now = getCustomTime();
    const diferencia = calcularDiferencia(fecha, time.hora_inicio, now).split(':');

    const segundos = Number(diferencia[0]) * 3600 + Number(diferencia[1]) * 60 + Number(diferencia[2]);

    return Math.floor(segundos * 100 / segundos_totales);
}

function dailyMessage() {
    const messages = [
        '¡Buenos días! 🌞 Hoy es una nueva oportunidad para avanzar, aprender y superar cualquier desafío.',
        '🍀🍀Recuerda que cada pequeño paso cuenta, y tienes la capacidad para lograr grandes cosas. 🍀🍀',
...
function updateCustomCircle() {
    const time = JSON.parse(localStorage.getItem(temporizador))?.pop();

    const percentage = getPercentage(time) || 0;

    drawCustomCircle('circleCanvas', {
        scale: 0.3,
        baseRadius: 100,
...
window.addEventListener('load', function () {
    let rj = new TimeCounter('iniciar', 'finalizar', temporizador);
    rj.init();

    this.setInterval(updateCustomCircle, 1000);

    let fm = new FileManager(temporizador);

    document.getElementById('daily-messages').innerHTML = dailyMessage();
});